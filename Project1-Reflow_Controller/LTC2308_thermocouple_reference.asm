; Warren & Gabriel: This code is for converting hex2bcd values from the thermocouple and temperature sensor and ouputting it
; on the hex displays of the De1
...
x: ds 4
y: ds 4
sensor: ds 4  ; temperature sensor used as cold junction on the breadboard
thermocouple: ds 4 
bcd: ds 5
;edited 
...
$include(math32.inc) ; declaring math32 functions to do conversions
...
Display_BCD:
	...
	mov a, R3  									; taking the higher 4 bits of the register R3
	swap a
	anl a, #0x0f
	movc a, @a+dptr
	mov HEX3, a
	...
	mov a, R3
	anl a, #0x0f 								; taking the lower 4 bits of the register R3
	movc a, @a+dptr
	mov HEX2, a
	
	mov a, R2
	swap a
	anl a, #0x0f  								; taking the uppwer 4 bits of the register R2
	movc a, @a+dptr
	mov HEX1, a
	
	mov a, R2 									; taking the lower 4 bits of the register
	anl a, #0x0f
	movc a, @a+dptr
	mov HEX0, a
...
;////////////////////////////////////////////////////////
; reading from channel 0 the hex values and storing it into sensor variable
getting_sensor_temperature: ;
	...
	mov R2, #1
	lcall MyDelay
	clr a ; read the channel to convert from the switches
	mov b, a
	lcall LTC2308_RW  ; Read the channel from the ADC
...
getting_thermocouple_temperature:
; adding the sensor (room temprature) in hex and doing conversions from thermocouple
    mov a, #2
	mov b, a
	lcall LTC2308_RW  ; Read the channel from the ADC
	...
	mov R2, #1
	lcall MyDelay
	mov a, #2 ; read the channel to convert from the switches
	mov b, a
	lcall LTC2308_RW  ; Read the channel from the ADC
	...
	mov thermocouple+0, x+0
	mov thermocouple+1, x+1

 
 ;/////////////////////////////

 add_cold_junction:
; this routine comes directly after getting_thermocouple_temperature to add the hex value of sensor and hex value of thermocouple , values moved into R0 and R1
    mov x+0, sensor+0
	mov x+1, sensor+1
	mov x+2, sensor+2
	mov x+3, sensor+3

	mov y+0, thermocouple+0
	mov y+1, thermocouple+1
	mov y+2, thermocouple+2
	mov y+3, thermocouple+3
...
forever:
	...
	lcall getting_sensor_temperature ; convert voltage to temperature subroutine
	;lcall getting_thermocouple_temperature

	; need to switch channel to get the thermocouple temperature

	lcall hex2bcd16   ; Convert to hex to bcd for outputted temperature from the thermocouple to the De1
